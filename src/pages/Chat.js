...
// import {query, collection, where, orderBy, onSnapshot, doc, setDoc, getDoc, updateDoc, arrayUnion, arrayRemove, limit} from 'firebase/firestore'
...
// import firebase to store server timestamp
...
export default function Chat() {
    ...
    // const query = usersCollectionRef.orderBy('createdAt').limit(25)
    ...
    return (
        <div>
            {msg && msg.map(({ id, text, uid }) => (
                <div key={id} className={`flex flex-col ${uid === user.uid ? 'items-end' : 'items-start'}`}>
                    <p>
                        {text}
                    </p>
                </div>
